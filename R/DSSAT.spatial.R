...
DSSAT.spatial <- function(template, label, raw.dir="./DATA/", force.redo=F){
  raw.dir <- path.expand(raw.dir)
  
  cat("Extracting and loading the NRCS soils data \n")
  SSURGO <- getSSURGO(template=template, label=label, raw.dir=paste(raw.dir,"RAW/SSURGO/",sep=''), extraction.dir=paste(raw.dir,"EXTRACTIONS/SSURGO/",sep=''), force.redo=force.redo)
  
  cat("Extracting and loading the NHD hydrology data \n")
  NHD <- getNHD(template=template, label=label, raw.dir=paste(raw.dir,"RAW/NHD/",sep=''), force.redo=force.redo)
  
  cat("Extracting and loading the NED elevation data, and draining the dammed DEM \n")
  NED <- extractNED(template=template, label=label, raw.dir=paste(raw.dir,"NED/",sep=''), res="1", drain=T, NHD.raw.dir=paste(raw.dir,"NHD/",sep=''), NRCS.raw.dir=paste(raw.dir,"NRCS/",sep=''), force.redo=force.redo)
