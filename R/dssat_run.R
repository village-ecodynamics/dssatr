dssat_run <- function(name,
  ...
  ## FIELDS ##
  fields <- 1:length(soils) %>%
    lapply(function(i){
      paste(sprintf("%2d", i)," ",soils[[i]]," ",weather,"   -99     0 DR000     0     0 00000  -99  ",sprintf("%4d", -99),"  ",sprintf("%010s", soils[[i]])," RAINFED",sep='')
    }) %>%
    unlist()
